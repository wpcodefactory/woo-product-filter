...
( function ( $ ) {
	...
	function goTo( item ) {
		$( 'html,body' ).animate( { 'scrollTop': $( item ).offset().top - 30 }, 1000 );
	...
	function AdminPage() {
		...
		this.$multiSelectFields = [
			...
			'f_exclude_terms[]',
			'f_cglist[][]',
			'f_search_by_attributes_list[]',
			'f_additional_attributes_list[]'
		];
	...
	AdminPage.prototype.setupPriceByHands = (
		function () {
			var _this = this.$obj,
				options = {
					modal: true
					...
					, buttons: {
						OK: function () {
							var emptyInput = false,
								options = '',
								range = $( '#wpfSetupPriceRangeByHand .wpfRangeByHand' );
							...
							if ( !emptyInput ) {
								...
								range.each( function () {
									...
									if ( minValue != 'i' && (
										minValue === false || from == 'i' || from < minValue
									) ) {
										minValue = from;
									}
									if ( maxValue != 'i' && (
										maxValue === false || to == 'i' || to > maxValue
									) ) {
										maxValue = to;
									}
									options += from + ',' + to;
								...
								var filterBlock = $( '.wpfFiltersBlock .wpfFilter[data-filter="wpfPriceRange"]' ),
									preselect = (
										minValue == 'i' || minValue === false ? '' : 'wpf_min_price=' + minValue
									);
								filterBlock.find( 'input[name="f_range_by_hands_values"]' ).val( options );
								if ( maxValue != 'i' && maxValue !== false ) {
									preselect += (
										             preselect.length ? ';' : ''
									             ) + 'wpf_max_price=' + maxValue;
								}
			...
			var $container = $( '<div id="wpfSetupPriceRangeByHand"></div>' ).dialog( options );

			$( 'body' ).off( 'click', '.wpfRangeByHandSetup' ).on( 'click', '.wpfRangeByHandSetup', function ( e ) {
				e.preventDefault();
				var appendTemplate = '',
					priceRange = $( 'input[name="f_range_by_hands_values"]' ).val(),
					template = $( '.wpfRangeByHandTemplate' ).clone().html(),
					templAddButton = $( '.wpfRangeByHandTemplateAddButton' ).clone().html();
				$container.empty();
				...
				if ( priceRange.length <= 0 ) {
					...
					var k = 0;
					$( '#wpfSetupPriceRangeByHand input' ).each( function () {
						var input = $( this );
			...
			$( 'body' ).off( 'click', '.wpfAddPriceRange' ).on( 'click', '.wpfAddPriceRange', function ( e ) {
				e.preventDefault();
				var templates = $( '.wpfRangeByHandTemplate' ).clone().html();
				$( templates ).insertBefore( '.wpfRangeByHandAddButton' );
				sortablePrice();
			} );

			$( 'body' ).off( 'click', '.wpfRangeByHandRemove' ).on( 'click', '.wpfRangeByHandRemove', function ( e ) {
				e.preventDefault();
				var _this = $( this );
			...
			function sortablePrice() {
				$( "#wpfSetupPriceRangeByHand" ).sortable( {
					//containment: "parent",
					cursor: "move",
					axis: "y",
	...
	AdminPage.prototype.initColorPicker = (
			...
			colorResult.wpColorPicker( {
				...
				change: function ( event, ui ) {
					var color = ui.color.toString(),
						wrapper = $( event.target ).closest( '.woobewoo-color-picker' ),
	...
	AdminPage.prototype.eventsAdminPage = (
		function () {
			var _thisObj = this.$obj,
				$mainTabsContent = $( '.row-tab' ),
				$mainTabs = $( '.wpfSub.tabs-wrapper.wpfMainTabs .button' ),
				$currentTab = $mainTabs.filter( '.current' ).attr( 'href' ),
				$form = $( '#wpfFiltersEditForm' );
			...
			$( '.sub-tab a' ).off( 'click' ).on( 'click', function ( e ) {
				e.preventDefault();
				var $this = $( this ),
			...
			_thisObj.initColorPicker( $( '.row-tab:not(#row-tab-filters) .woobewoo-color-result' ) );

			$( '.wpfFiltersTabContents' ).off( 'change', '.woobewoo-color-result-text' ).on( 'change', '.woobewoo-color-result-text', function () {
				var $this = $( this ),
					color = $this.val(),
					$picker = $this.closest( '.woobewoo-color-picker' );
				if ( color == '' ) {
					$picker.find( '.wp-picker-clear' ).trigger( 'click' );
				} else {
					$picker.find( '.woobewoo-color-result' ).wpColorPicker( 'color', $this.val() );
				}
			} );

			$form.find( 'input[name="settings[filter_loader_icon_color]"]' ).off( 'color-change' ).on( 'color-change', function ( e ) {
				$( '.woobewoo-filter-loader' ).css( { color: $( this ).val() } );
			} );


			$( '.chooseLoaderIcon' ).off( 'click' ).on( 'click', function ( e ) {
				e.preventDefault();
				if ( typeof (
			...
			$( document ).on( 'click', '.wpfEditCategory', function ( e ) {
				e.preventDefault();
				const li = $( this ).closest( '.editablecat' );
			...
			//category name change
			$( 'body' ).off( 'click', '#buttonDelete' ).on( 'click', '#buttonDelete', function ( e ) {
				e.preventDefault();
				var deleteForm = confirm( "Are you sure you want to delete filter?" );
				if ( deleteForm ) {
					var id = $( '#wpfFiltersEditForm' ).attr( 'data-table-id' );
					...
					if ( id ) {
						...
						$.ajax( {
							...
							success: function ( res ) {
								var redirectUrl = $( '#wpfFiltersEditForm' ).attr( 'data-href' );
								if ( !res.error ) {
			...
			$( '#wpfCopyTextCodeExamples' ).on( 'change', function ( e ) {
				var optName = $( this ).val();
				switch ( optName ) {
					case 'shortcode' :
						$( '.wpfCopyTextCodeShowBlock' ).addClass( 'wpfHidden' );
						$( '.wpfCopyTextCodeShowBlock.shortcode' ).removeClass( 'wpfHidden' );
						break;
					case 'phpcode' :
						$( '.wpfCopyTextCodeShowBlock' ).addClass( 'wpfHidden' );
						$( '.wpfCopyTextCodeShowBlock.phpcode' ).removeClass( 'wpfHidden' );
						break;
					case 'shortcode_product' :
						$( '.wpfCopyTextCodeShowBlock' ).addClass( 'wpfHidden' );
						$( '.wpfCopyTextCodeShowBlock.shortcode_product' ).removeClass( 'wpfHidden' );
						break;
					case 'phpcode_product' :
						$( '.wpfCopyTextCodeShowBlock' ).addClass( 'wpfHidden' );
						$( '.wpfCopyTextCodeShowBlock.phpcode_product' ).removeClass( 'wpfHidden' );
						break;
				}
			} );
			...
			$( 'body' ).off( 'focus', '.wpfFilter div > input' ).on( 'focus', '.wpfFilter div > input', function () {
				if ( typeof $( this ).attr( 'placeholder' ) !== 'undefined' && $( this ).attr( 'placeholder' ).length > 0 ) {
					$( this ).attr( 'data-placeholder', $( this ).attr( 'placeholder' ) );
					$( this ).attr( 'placeholder', '' );
				}
			} );
			$( 'body' ).off( 'blur', '.wpfFilter div > input' ).on( 'blur', '.wpfFilter div > input', function () {
				$( this ).attr( 'placeholder', $( this ).attr( 'data-placeholder' ) );
			} );

			var settingsValues = $( '.wpfFiltersTabContents' );
				...
				if ( childrens.length > 0 ) {
					childrens.each( function () {
						...
						if ( elem.prop( 'nodeName' ).toLowerCase() == 'select' ) {
							if ( (
								     '' == values || toeInArrayWpf( elem.val(), values )
							     ) && (
								     '' == noValues || !toeInArrayWpf( elem.val(), noValues )
							     ) ) {
							...
							if ( (
								     ...
								     valueWrapper.length == 0 || !valueWrapper.hasClass( 'wpfHidden' )
							     ) ) {
								child.removeClass( 'wpfHidden' );
							} else {
								child.addClass( 'wpfHidden' );
					...
					childrens.each( function () {
						...
						if ( elem.prop( 'nodeName' ).toLowerCase() == 'select' ) {
							var jq_selector = parentsNames // 'settings[display_hide_button] settings[display_hide_button_mobile]'
								...
								.join( ',' );
							var all_no = $( jq_selector ).length === parentsNames.split( ' ' ).length
							if ( all_no ) {
								child.addClass( 'wpfHidden' );
							} else {
								child.removeClass( 'wpfHidden' );
			...
			settingsValues.off( 'change wpf-change', 'select' ).on( 'change wpf-change', 'select', function () {
				var elem = $( this ),
					...
					hidden = elem.closest( '.settings-value' ).hasClass( 'wpfHidden' ) || (
						elem.closest( '.wpfHidden', '.sub-tab-content' ).length && !elem.closest( '.wpfHidden', '.sub-tab-content' ).is( '.wpfOptions' )
					),
					name = elem.attr( 'name' ),
					filterBlock = elem.closest( '.wpfFilter' ),
					block = filterBlock.length ? filterBlock : settingsValues,
			...
			$( 'div[data-filter="wpfBrand"] div[data-parent="f_custom_tags"]' ).each( function () {
			...
			$( document ).keydown( function ( e ) {
				if (
					...
					if (
						$multiBlock.length === 1 &&
						$multiBlock.find( '.ts-wrapper' ).hasClass( 'focus' )
					) {
						const ctrlAttr = $multiBlock.attr( 'data-ctrl-a' ) !== '1',
	...
	AdminPage.prototype.getPreviewAjax = (
		function ( wait ) {
			var _this = this.$obj;
			if ( _this.wpfWaitLoad ) {
				return;
			}
			if ( _this.isElementorEditMode ) {
				return;
			}
			...
			$form.sendFormWpf( {
				...
				appendData: {
					mod: 'woofilters',
					action: 'woobewoo_pf_draw_filter_ajax',
					wpfNonce: woobewoo_pf_admin_ajax_object.nonce
				},
				onSuccess: function ( res ) {
					if ( !res.error ) {
	...
	AdminPage.prototype.eventsFilters = (
		function () {
			var _this = this.$obj,
				_noOptionsFilters = this.$noOptionsFilters,
				wpfGetPreviewInit = false;

			$( '.wpfMainWrapper' ).find( 'select[multiple]' ).multiselect( {
				columns: 1,
				placeholder: 'Select options'
			} );

			$( 'document' ).ready( function () {
				$( '.color-group' ).trigger( 'color-group' );
			} );
			if ( WPF_DATA.isWCLicense ) {
				$( '.wpfProLabel a' ).attr( 'href', '' );
			}

			$( "body" ).off( 'change', "[name='f_show_inputs']" ).on( 'change', "[name='f_show_inputs']", function ( e ) {
				e.preventDefault();
				if ( $( this ).prop( 'checked' ) ) {
					if ( $( "[name='f_skin_type']" ).val() == 'default' ) {
						$( ".f_show_inputs_enabled_tooltip" ).show();
					} else {
						$( ".f_show_inputs_enabled_tooltip" ).hide();
					}
					$( ".f_show_inputs_enabled_position" ).show();
					$( ".f_show_inputs_enabled_currency" ).show();
				} else {
					var curSlider = $( '[name="f_price_show_currency_slider"]' );
					$( ".f_show_inputs_enabled_tooltip" ).hide();
					$( ".f_show_inputs_enabled_position" ).hide();
					$( ".f_show_inputs_enabled_currency" ).hide();
					if ( curSlider.length == 0 || !curSlider.prop( 'checked' ) ) {
						$( "[name='f_currency_position']" ).val( 'before' );
						$( "[name='f_currency_show_as']" ).val( 'symbol' );
					}
					$( "[name='f_price_tooltip_show_as']" ).prop( "checked", false );
					$( "[name='f_price_tooltip_show_as']" ).attr( "checked", false );
				}
			} );

			$( "body" ).off( 'change', "[name='f_skin_type']" ).on( 'change', "[name='f_skin_type']", function ( e ) {
				e.preventDefault();
				if ( $( this ).val() == 'default' ) {
					if ( $( "[name='f_show_inputs']" ).prop( "checked" ) ) {
						$( ".f_show_inputs_enabled_tooltip" ).show();
					} else {
						$( ".f_show_inputs_enabled_tooltip" ).hide();
					}
			...
			$( 'body' ).off( 'change wpf-update', '.wpfFiltersTabContents select, .wpfFiltersTabContents input' ).on( 'change wpf-update', '.wpfFiltersTabContents select, .wpfFiltersTabContents input', function ( e ) {
				if ( $( this ).closest( 'div[data-no-preview="1"]' ).length == 0 ) {
					_this.getPreviewAjax();
				}
			} );

			$( "body" ).off( "change", '.wpfFiltersTabContents [name="f_hidden_attributes"]' ).on( "change", '.wpfFiltersTabContents [name="f_hidden_attributes"]', function ( e ) {
				if ( !$( this ).closest( ".wpfFiltersBlockPreview" ).length ) {
					_this.getPreviewAjax();
				}
			} );
			...
			$( "body" )
				.off( "change", '#wpfFiltersEditForm [name="f_hide_taxonomy"]' )
				.on( "change", '#wpfFiltersEditForm [name="f_hide_taxonomy"]', function ( e ) {

					var mList = $( this ).closest( '.wpfFilter' ).find( 'select[name="f_mlist[]"]' ),
						parentCats = mList.data( 'parents' );

					mList.find( 'option' ).show();
					if ( $( this ).is( ':checked' ) ) {
						mList.closest( '.wpf-multi-select' ).attr( 'data-ctrl-a', '0' );
			...
			$( "body" )
				.off( "change", '#wpfFiltersEditForm select[name="f_mlist[]"], #wpfFiltersEditForm select[name="f_exclude_terms[]"]' )
				.on( "change", '#wpfFiltersEditForm select[name="f_mlist[]"], #wpfFiltersEditForm select[name="f_exclude_terms[]"]', function () {
					_this.getPreviewAjax();
				} );

			$( '#wpfChooseFilters' ).off( 'change' ).on( 'change', function () {
				var option = $( '#wpfChooseFilters option:selected' ),
					variants = $( '#wpfChooseFiltersBlock [data-option]' ).addClass( 'wpfHidden' );
				variants.filter( '[data-option="' + option.attr( 'data-available' ) + '"]' ).removeClass( 'wpfHidden' );
			} );
			$( '#wpfChooseFilters option' ).each( function () {
				var $opt = $( this );
			...
			function resetEnabledFilters() {
				var filterSelect = $( '#wpfChooseFilters' ),
					filtersBlock = $( '.wpfFiltersBlock' );
				...
				if ( firstEnabled.length ) {
					firstEnabled.first().prop( 'selected', true );
				}
				filterSelect.trigger( 'change' );

				if ( filtersBlock.find( '.wpfFilter' ).length ) {
					filtersBlock.removeClass( 'wpfHidden' );
				} else {
					filtersBlock.addClass( 'wpfHidden' );
				}
			...
			$( '#wpfAddFilterButton' ).off( 'click' ).on( 'click', function ( e ) {
				e.preventDefault();
				var option = $( '#wpfChooseFilters option:selected' );
				if ( option.length == 0 || option.attr( 'data-enabled' ) != '1' || option.attr( 'data-available' ) != 'add' ) {
					return;
				}
			...
			$( '.wpfFiltersBlock' ).off( 'click', '.wpfFilter a.wpfDelete' ).on( 'click', '.wpfFilter a.wpfDelete', function ( e ) {
				e.preventDefault();
				$( this ).closest( '.wpfFilter' ).remove();
				resetEnabledFilters();
				_this.getPreviewAjax();
			} );

			//show / hide filter options
			$( '.wpfFiltersBlock' ).off( 'click', '.wpfFilter a.wpfToggle' ).on( 'click', '.wpfFilter a.wpfToggle', function ( e ) {
				e.preventDefault();
				var el = $( this ),
					i = el.find( 'i' ),
				...
				if ( i.hasClass( 'fa-chevron-down' ) ) {
					var elOther = $( '.wpfFilter a.wpfToggle i.fa-chevron-up' ),
			...
			$( document ).off( 'change wpf-dependent', '.wpfFiltersBlock input.toggle, #row-tab-options input.toggle' ).on( 'change wpf-dependent', '.wpfFiltersBlock input.toggle, #row-tab-options input.toggle', function () {
				var parent = $( this ).closest( '.wpfFilter, #row-tab-options' ),
					name = $( this ).attr( 'name' ),
					...
					if ( isActive ) {
						if ( elem.hasClass( 'wpfDependencyHidden' ) ) {
							...
							if ( elem.closest( '.wpfAddTypeControl' ).length ) {
								...
								if ( $blockType.length && !elem.closest( '.wpfAddTypeControl' ).is( '[data-type~="' + $blockType.val() + '"]' ) ) {
									hide = true;
								}
							}
							if ( !hide ) {
							...
							elem.each( function () {
								var el = $( this );
								el.css( 'display', (
									el.hasClass( 'settings-value' ) ? 'flex' : 'block'
								) );
						...
						if ( dependentCheckboxes.length > 0 ) {
							dependentCheckboxes.each( function () {
								$( this ).prop( 'checked', false ).trigger( 'wpf-dependent' );
							} );
						}
			...
			$( '.wpfFiltersTabContents' ).off( 'change', '.wpfTypeSwitchable select' ).on( 'change', '.wpfTypeSwitchable select', function ( e ) {
				var el = $( this ),
					name = el.attr( 'name' );
			...
			$( '.wpfFiltersBlock' ).off( 'change wpf-change', 'select[name="f_frontend_type"]' ).on( 'change wpf-change', 'select[name="f_frontend_type"]', function ( e ) {
				e.preventDefault();
				var el = $( this ),
					value = el.val(),
				...
				filter.find( '.wpfTypeSwitchable[data-type~="' + value + '"]' ).each( function () {
					...
					if ( $( this ).hasClass( 'wpfDependencyHidden' ) ) {
						let parentSwitch = $( this ).data( 'parent-switch' );
				...
				if ( value === 'colors' ) {
					$( '.color-group', filter ).trigger( 'color-group' );
				}
			} );
			...
			function displayFiltersTab() {
				$( '.wpfFiltersBlock' ).html( '' );
				try {
					var filters = JSON.parse( $( 'input[name="settings[filters][order]"]' ).val() ),
			...
			$( "body" ).off( 'change', 'select[name="f_list"]' ).on( 'change', 'select[name="f_list"]', function ( e ) {
				e.preventDefault();
				_this.fListChanged( $( this ), 'change' );
			} );

			$( '.wpfFiltersBlock' ).off( 'change', '.wpfAutomaticOrByHand input[type="checkbox"]' ).on( 'change', '.wpfAutomaticOrByHand input[type="checkbox"]', function () {
				var _this = $( this ),
					checked = _this.prop( 'checked' );

				$( '.wpfAutomaticOrByHand' ).not( '[data-value="' + _this.closest( '.wpfAutomaticOrByHand' ).attr( 'data-value' ) + '"]' ).find( 'input' ).prop( 'checked', !checked );
				$( '.wpfAutomaticOrByHand input[type="checkbox"]' ).trigger( 'wpf-change' );
			} );

		}
	);

	AdminPage.prototype.setAttrTerms = (
		function ( mlist, slug ) {
			var _this = this,
				options = $( '.wpfAttributesTerms input[name="attr-' + slug + '"]' );
			if ( typeof (
				options
			) == 'undefined' || options.length == 0 ) {
				return;
			}
			...
			if ( filterId ) {
				...
				settings = (
					_this.filtersSettings && filterNum in _this.filtersSettings ? _this.filtersSettings[ filterNum ][ 'settings' ] : []
				);
			}
	...
	AdminPage.prototype.fListChanged = (
		function ( _this, event ) {
			...
			if ( attrSlug != 0 ) {
				var terms = $( '.wpfAttributesTerms input[name="attr-' + attrSlug + '"]' );
				if ( typeof (
					terms
					...
					$.sendFormWpf( {
						...
						onSuccess: function ( res ) {
							if ( !res.error && res.data.terms && res.data.keys ) {
								$( '.wpfAttributesTerms' ).append( '<input type="hidden" name="attr-' + attrSlug + '" data-order="' + res.data.keys + '" value="' + res.data.terms + '">' );
			...
			if ( frontendType.val() === 'colors' ) {
				$( '.color-group', _this.closest( '.wpfFilter' ) ).trigger( 'color-group' );
			}
	...
	AdminPage.prototype.wpfAddFilter = (
		function ( id, uniqId, settings ) {
			var _this = this,
				_noOptionsFilters = this.$noOptionsFilters,
				wpfGetPreviewInit = false,
				template = $( '.wpfOptionsTemplate .wpfFilterOptions[data-filter="' + id + '"]' );
			...
			var filterUniqId = uniqId ? uniqId : 'wpf_' + Math.random().toString( 36 ).substr( 2, 9 ),
				...
				blockTemplate = $( '.wpfTemplates .wpfFiltersBlockTemplate' )
					.clone()
			...
			$( '.wpfFiltersBlock' ).append( blockTemplate );
			...
			if ( id == 'wpfPrice' ) {
				...
				$( '#wpfContainerSortBy a.js-wpfMove' ).on( 'click', function ( e ) {
	...
	AdminPage.prototype.saveFilters = (
		function () {
			var _this = this.$obj,
				filtersArr = [],
				i = 0,
				preselect = '',
				attrNames = $( '.wpfAttributesTerms input[name="attr_filternames"]' ).val();
			if ( typeof (
				attrNames
			) == 'undefined' || attrNames.length == 0 ) {
				return;
			}
			try {
			...
			$( '.wpfFilter' ).not( '.wpfFiltersBlockTemplate' ).each( function () {
				var valueToPush = {},
					filter = $( this ),
					id = 'wpfFilter' + i,
					items = {},
					...
					filterName = $( '#wpfChooseFilters option[value="' + filterId + '"]' ).attr( 'data-filtername' );
				...
				if ( filterId == 'wpfCategory' ) {
					if ( !items[ 'f_hide_taxonomy' ] ) {
						...
						if ( type == 'multi' || type == 'buttons' || type == 'text' ) {
							filterName += '_list';
						}
					}
				}
				...
				filter.find( 'input[data-preselect-flag="1"]' ).each( function () {
					...
					if ( elm.type === 'checkbox' && elm.checked ) {
						...
						if ( 'f_preselect' in items ) {
							...
							if ( mlist.length ) {
								switch ( filterId ) {
									case 'wpfCategory':
										preValue = filterName + (
											items[ 'f_extend_parent_filtering' ] ? '' : '_list'
										) + '=' + (
											           items[ 'f_multi_logic' ] == 'or' ? mlist.replace( /,/g, '|' ) : mlist
										           );
										break;
									case 'wpfTags':
									case 'wpfAttribute':
										var fn = (
											items[ 'f_query_logic' ] == 'not_in' ? 'pr_' + filterName : filterName
										);
										preValue = fn + '=' + (
											items[ 'f_query_logic' ] == 'or' ? mlist.replace( /,/g, '|' ) : mlist
										);
										break;
								}
							} else {
						...
						if ( typeof (
							_this.addPreselectsPro
						) == 'function' ) {
							preselect = _this.addPreselectsPro( preselect, items, filterId );
						}
					}
				} );
			...
			$( 'input[name="settings[filters][order]"]' ).val( filtersJson );
			$( 'input[name="settings[filters][preselect]"]' ).val( preselect.length ? preselect.slice( 0, - 1 ) : '' );
			if ( typeof (
				_this.saveFiltersPro
			) == 'function' ) {
				_this.saveFiltersPro();
			}
	...
	function wpfSortBy_MoveRow( e, obj ) {
		...
		var $row = $( obj ).closest( '.js-wpf-row-move' ), _container = $row[ 0 ].parentNode,
			_insertBeforeElement = null, $element = null, strLastClass = 'js-last-item';
		...
		if ( obj.classList.contains( 'wpfMoveUp' ) ) {
			...
			(
				$element.length > 0
			) ? $element = $element.next() : $row.addClass( strLastClass );

			_insertBeforeElement = (
				$element.length == 0 && $row.hasClass( strLastClass )
			) ? _container.children[ 0 ] : $element[ 0 ];
			(
				$element.length == 0
			) ? $row.addClass( strLastClass ) : '';
		}

		_container.insertBefore( $row[ 0 ], _insertBeforeElement );
		(
			$row.next().length > 0
		) ? $row.removeClass( strLastClass ) : $row.addClass( strLastClass );
