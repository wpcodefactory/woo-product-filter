...
class WoofiltersViewWpf extends ViewWpf
{
	...
	public function generatePriceFilterHtml($filter, $filterSettings, $blockStyle, $key = 1, $viewId = '')
	{
		...
		$html =
			...
			'<div id="wpfSliderRange" class="wpfPriceFilterRange"></div>' . $this->generatePriceInputsHtml($settings) .
			'</div>';
		$html .= '</div>';
		return $html;
	}
	...
	public function generatePerfectBrandFilterHtml($filter, $filterSettings, $blockStyle, $key = 1, $viewId = '')
	{
		...
		if ($hiddenBrands) {
			if (! $includeBrandId && $excludeIds) {
				$logic     = 'not';
				$notValues = '" data-not-ids="' . esc_attr(implode(',', $excludeIds));
			}
		}
		...
		foreach ($options as $enabledOption) {
			...
			if (isset($CustomOptions[$enabledOption])) {
				...
				/* if ($fieldType === 'checkbox') {
					...
					$htmlOpt = '<div class="wpfFilterOptionsWrapper"><ul class="d-flex flex-column">';
					...
					foreach ($fieldValues as $value) {
						...
						$htmlOpt .= '<li class="wpfCheckbox mb-2" data-term-id="' . esc_attr($term_id) . '" data-term-slug="' . esc_attr($term_slug) . '">';
						$htmlOpt .=   '<label class="encoderCheckbox d-flex items-center">';
						$htmlOpt .= '<input type="checkbox" name="' . esc_attr($name) . '[]" value="' . esc_attr($term_id) . '"' . $checked . '>';

						$htmlOpt .= '<span>' . esc_html($value) . '</span>';
						$htmlOpt .=   '</label>';
						$htmlOpt .= '</li>';
					}
				...
				if ($fieldType === 'radio') {
					...
					foreach ($fieldValues as $value) {
						...
						$htmlOpt .= '<li class="wpfRadio mb-2" data-term-id="' . esc_attr($term_id) . '" data-term-slug="' . esc_attr($term_slug) . '">';
						$htmlOpt .=   '<label class="encoderRadio d-flex items-center">';
						$htmlOpt .= '<input type="radio" name="' . esc_attr($name) . '" value="' . esc_attr($term_id) . '"' . $checked . '>';
		...
		foreach ($filterItemList as $filterItem) {
			...
			if ('dropdown' === $type || 'mul_dropdown' === $type) {
				...
				if (! $hideParent || ($isChild && (! $hideAllParent || ! $hasChildren))) {
					...
					$displayName = '<' . $titleTag . ' class="wpfFilterTaxNameWrapper">' . esc_html($displayName) . '</' . $titleTag . '>';
