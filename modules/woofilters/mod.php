...
class WoofiltersWpf extends ModuleWpf
{
	...
	public function loadProductsFilter($q)
	{
		...
		// set preselects (filter default values from config)
		$mode       = 'preselect';
		$preselects = $this->getPreselectedValue();

		// When slug format is enabled: merge current URL GET into preselects so slug URLs apply to main shop query on page load.

		$urlParams = ReqWpf::get('get');

		if (is_array($urlParams)) {
			$ignoreKey = array('wpf_count', 'wpf_fbv', 'wpf_dpv', 'wpf_skip', '_');
			foreach ($urlParams as $key => $val) {
				if (in_array($key, $ignoreKey, true) || $val === '' || $val === null) {
					continue;
				}
				if (
					'orderby' === $key || strpos($key, 'wpf_') === 0 || strpos($key, 'product_tag') === 0
					|| strpos($key, 'product_cat') === 0 || strpos($key, 'pr_') === 0
					|| strpos($key, 'pa_') === 0 || strpos($key, 'pwb-') === 0 || strpos($key, 'pc_') === 0
				) {
					$preselects[$key] = $val;
				}
			}
	...
	public function addAdminTab($tabs)
	{
		...
		$tabs[$this->getCode()]             = array(
			'label'      => esc_html__('All Filters', 'woo-product-filter'),
			'callback'   => array($this, 'getTabContent'),
			'fa_icon'    => 'fa-list',
			'sort_order' => 20,
		);
